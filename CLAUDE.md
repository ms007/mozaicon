...
- **Scoped checks in the loop, full `pnpm check` only before committing.** During iteration run `pnpm check:scoped <files you edited>` — it runs Prettier + ESLint (cached) + `vitest related` on just those files (~5s vs ~55s) and skips `tsc` because the LSP `<new-diagnostics>` push already surfaces new type errors after every edit. You know your exact changed-set, so pass the files explicitly. Run the **full** `pnpm check` once before committing — it stays the quality gate (whole-program `tsc` + full suite catch anything the import-graph-scoped run misses). Don't run the full check after every single `Edit`.
